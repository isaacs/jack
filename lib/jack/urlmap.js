...
var URLMap = exports.URLMap = function(map, options) {
    ...
    return function(env) {
        ...
        for (var i = 0; i < mapping.length; i++)
        {
            ...
            if (
                // Make sure that requested host and path match the mapping's host and path.
                (
                    // mapping host matches HTTP_HOST
                    host === hHost
                    // or, mapping host matches server name
                    || host === sName
                    // or, no mapping host, but HTTP_HOST matches server name ??
                    // FIXME: This seems strange.
                    || ( host === null && (hHost === sName || hHost === sName+":"+sPort) )
                ) && (
                    // mapping location matches the start of the requested path
                    location === path.substring(0, location.length)
                ) && (
                    // make sure that /a doesn't match /apple, but can match /a/pple
